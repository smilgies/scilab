...
// NEW: r is now Boolean, not a number any more
// NEW: reciprocal lattice search via sg_info

function plot_pars=calc_spots(det_info,sg_info,plot_info)

    // support for D1 area detector maps and G2 RSM scans
    ...
    for refl=-1:2:refl_flag
        ...
        case "area"
            ...
            error("pos5: bad scan_type: "+scan_type)
    ...
    if size(spots,"r")==0 then
        ...
        messagebox("pos6 warning: no spots in range")
    end
    ...
    if scan_type=="area" then
        ...
        // hn kn ln x  z  nu del Q  qz qq  qq0 kfx kfz

        // sort spots by qq and write to file
        spots=sort_by_col(spots,10);    
        fprintfMat(TMPDIR+"\spots.dat",spots,"%f","spots.dat: hn kn ln x z nu del Q qz qq qq0"); 

        if refl_flag==1 then
            // sort rspots by qq and write to file
            rspots=sort_by_col(rspots,10);
            fprintfMat(TMPDIR+"\rspots.dat",rspots,"%f","rspots.dat: hn kn ln x z nu del Q qz qq qq0"); 
        end
        
    elseif scan_type=="GIRSM" then
        // numerical output - spots columns:
        // 1  2  3  4  5   6 7  8  9  
        // hn kn ln nu del Q qz qq qq0

        // sort spots by qq and write to file
        spots=sort_by_col(spots,8);    
        fprintfMat(TMPDIR+"\spots.dat",spots,"%f","spots.dat: hn kn ln nu del Q qz qq qq0"); 
        if refl_flag==1 then
            // sort rspots by qq and write to file
            rspots=sort_by_col(rspots,8);
            fprintfMat(TMPDIR+"\rspots.dat",rspots,"%f","rspots.dat: hn kn ln nu del Q qz qq qq0");
        end
    else
