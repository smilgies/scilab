...
function plot_info=get_plot_info(plot_info);
    ...
    // remember previous label offset
    old_offset=plot_info(12) 
    ...
    if buffer~=[] then
        ...
        if yr==2023 then
            ...
            if h_label~=[] then
                ...
                old_pos=h_label.data;
                new_xpos=[old_pos(:,1)-old_offset+new_offset, old_pos(:,2)];
                h_label.data=new_xpos;
            end
            
            // reflected beam scattering
            h_rlabel=findobj("tag","rlabels");
            if h_rlabel~=[] then
                h_rlabel.font_style=decode_text(plot_info(9));
                h_rlabel.font_size=evstr(plot_info(10));
                h_rlabel.font_foreground=decode_color(plot_info(11));
                new_xpos=h_rlabel.data(:)-old_offset+new_offset;
        ...
        elseif yr>=2024 then
            ...
            h_label=findobj("tag","labels");
            if h_label~=[] then
                h_label.children.font_style=decode_text(plot_info(9));
                h_label.children.font_size=evstr(plot_info(10));
                h_label.children.font_foreground=decode_color(plot_info(11));
                // retrieve old label positions (x,z))
                old_pos=h_label.children.data
                new_xpos=[old_pos(:,1)-old_offset+new_offset, old_pos(:,2)];
                h_label.children.data=new_xpos;
            end
            
            // reflected beam scattering
            h_rlabel=findobj("tag","rlabels");
            if h_rlabel~=[] then
                h_rlabel.children.font_style=decode_text(plot_info(9));
                h_rlabel.children.font_size=evstr(plot_info(10));
                h_rlabel.children.font_foreground=decode_color(plot_info(11));
                new_xpos=h_rlabel.children.data-old_offset+new_offset;
